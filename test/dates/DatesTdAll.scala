...
object DatesTdAll {
  ...
  val `startDatesResponse-NoInitialPaymentDate`: StartDatesResponse = StartDatesResponse(
    initialPaymentDate = None,
    instalmentStartDate = InstalmentStartDate(LocalDate.parse(TdDates.`1stFeb2022`))
  )

  def startDatesResponse(
  ...
  val `startDatesResponse-WithInitialPaymentDate`: StartDatesResponse = StartDatesResponse(
    initialPaymentDate = Some(initialPaymentDate),
    instalmentStartDate = InstalmentStartDate(LocalDate.parse(TdDates.`1stFeb2022`))
  )
  val `startDatesResponseJson-NoInitialPaymentDate`: JsValue          = Json.parse("""{"instalmentStartDate":"2022-02-01"}""")
  val `startDatesResponseJson-WithInitialPaymentDate`: JsValue        =
    Json.parse("""{"initialPaymentDate":"2022-01-01", "instalmentStartDate":"2022-02-01"}""")
  ...
  val extremeDatesResponse: ExtremeDatesResponse                 = ExtremeDatesResponse(
    Some(initialPaymentDate),
    EarliestPaymentPlanStartDate(testDate),
    LatestPaymentPlanStartDate(testDate)
  )
  val extremeDatesNoUpfrontPaymentResponse: ExtremeDatesResponse =
    ExtremeDatesResponse(None, EarliestPaymentPlanStartDate(testDate), LatestPaymentPlanStartDate(testDate))
  val extremeDatesResponseJson: JsValue                          = Json.parse(
    """{"initialPaymentDate":"2022-01-01","earliestPlanStartDate":"2022-01-01","latestPlanStartDate":"2022-01-01"}"""
  )
  val extremeDatesNoUpfrontPaymentResponseJson: JsValue          =
    Json.parse("""{"earliestPlanStartDate":"2022-01-01","latestPlanStartDate":"2022-01-01"}""")
